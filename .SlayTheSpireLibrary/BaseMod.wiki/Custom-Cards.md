...
* `img` - the path to the img for this card (image path starts at the root of your `jar`) (250px x 190px); the path to your larger version of the img (500 x 380p) should be img + "_p" so if img is "my_card.png" then the portrait version should be at "my_card_p.png". For beta art, the files would be "my_card_b.png" and for the portrait version, "my_card_b_p.png".
...
# Registering
In order to use the methods below to add or remove cards you must implement `EditCardsSubscriber` in your main mod file and then override the `receiveEditCards` method. Inside that method is when you should add or remove cards.
