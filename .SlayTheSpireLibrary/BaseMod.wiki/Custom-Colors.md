...
The base game denotes color using the `AbstractCard.CardColor` enum so in order to add a new color to the game you must patch the enum using `ModTheSpire`'s enum patching feature in addition to registering it with **BaseMod**.
...
* `trailVfx` - Visual effects trail color
