...
extends AbstractCampfireOption {
    ...
    public RestOption(boolean active) {
        this.label = TEXT[0];
        this.usable = active;
        int healAmt = ModHelper.isModEnabled((String)"Night Terrors") ? (int)((float)AbstractDungeon.player.maxHealth * 1.0f) : (int)((float)AbstractDungeon.player.maxHealth * 0.3f);
        if (Settings.isEndless && AbstractDungeon.player.hasBlight("FullBelly")) {
            healAmt /= 2;
        }
        if (ModHelper.isModEnabled((String)"Night Terrors")) {
