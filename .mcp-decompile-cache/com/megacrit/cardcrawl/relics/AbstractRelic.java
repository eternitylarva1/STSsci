 ...
 *  com.megacrit.cardcrawl.core.GameCursor$CursorType
 ...
 *  com.megacrit.cardcrawl.helpers.GameDataStringBuilder
 ...
 *  com.megacrit.cardcrawl.helpers.ShaderHelper
 *  com.megacrit.cardcrawl.helpers.ShaderHelper$Shader
 ...
 *  com.megacrit.cardcrawl.localization.TutorialStrings
 *  com.megacrit.cardcrawl.monsters.AbstractMonster
 *  com.megacrit.cardcrawl.orbs.AbstractOrb
 *  com.megacrit.cardcrawl.relics.AbstractRelic$1
 ...
 *  com.megacrit.cardcrawl.ui.FtueTip$TipType
 *  com.megacrit.cardcrawl.ui.campfire.AbstractCampfireOption
 *  com.megacrit.cardcrawl.unlock.UnlockTracker
 *  com.megacrit.cardcrawl.vfx.FloatyEffect
 *  com.megacrit.cardcrawl.vfx.GlowRelicParticle
 *  com.megacrit.cardcrawl.vfx.SmokePuffEffect
 */
...
implements Comparable<AbstractRelic> {
    ...
    public AbstractRelic(String setId, String imgName, RelicTier tier, LandingSound sfx) {
        ...
        this.imgUrl = imgName;
        ImageMaster.loadRelicImg((String)setId, (String)imgName);
        this.img = ImageMaster.getRelicImg((String)setId);
        this.outlineImg = ImageMaster.getRelicOutlineImg((String)setId);
    ...
    public int getPrice() {
        switch (1.$SwitchMap$com$megacrit$cardcrawl$relics$AbstractRelic$RelicTier[this.tier.ordinal()]) {
            case 1: {
                return 300;
            }
            case 2: {
    ...
    public void reorganizeObtain(AbstractPlayer p, int slot, boolean callOnEquip, int relicAmount) {
        ...
        UnlockTracker.markRelicAsSeen((String)this.relicId);
    }
    ...
    public void relicTip() {
        if (TipTracker.relicCounter < 20 && ++TipTracker.relicCounter >= 1 && !((Boolean)TipTracker.tips.get("RELIC_TIP")).booleanValue()) {
            AbstractDungeon.ftue = new FtueTip(LABEL[0], MSG[0], 360.0f * Settings.scale, 760.0f * Settings.scale, FtueTip.TipType.RELIC);
            TipTracker.neverShowAgain((String)"RELIC_TIP");
        }
    }
    ...
    public void update() {
        ...
        if (!this.isDone) {
            if (this.isAnimating) {
                ...
                this.scale = this.hb.hovered ? Settings.scale * 1.5f : MathHelper.scaleLerpSnap((float)this.scale, (float)(Settings.scale * 1.1f));
            } else {
                this.scale = this.hb.hovered ? Settings.scale * 1.25f : MathHelper.scaleLerpSnap((float)this.scale, (float)Settings.scale);
            }
            if (this.isObtained) {
                if (this.rotation != 0.0f) {
                    this.rotation = MathUtils.lerp((float)this.rotation, (float)0.0f, (float)(Gdx.graphics.getDeltaTime() * 6.0f * 2.0f));
                }
                if (this.currentX != this.targetX) {
                    this.currentX = MathUtils.lerp((float)this.currentX, (float)this.targetX, (float)(Gdx.graphics.getDeltaTime() * 6.0f));
                    if (Math.abs(this.currentX - this.targetX) < 0.5f) {
                        this.currentX = this.targetX;
                    }
                }
                if (this.currentY != this.targetY) {
                    this.currentY = MathUtils.lerp((float)this.currentY, (float)this.targetY, (float)(Gdx.graphics.getDeltaTime() * 6.0f));
            ...
            if (this.hb.hovered && AbstractDungeon.topPanel.potionUi.isHidden) {
                ...
                this.scale = MathHelper.scaleLerpSnap((float)this.scale, (float)Settings.scale);
            }
    ...
    public void playLandingSFX() {
        switch (1.$SwitchMap$com$megacrit$cardcrawl$relics$AbstractRelic$LandingSound[this.landingSFX.ordinal()]) {
            case 1: {
    ...
    public int onPlayerGainedBlock(float blockAmount) {
        return MathUtils.floor((float)blockAmount);
    }
    ...
    public void renderInTopPanel(SpriteBatch sb) {
        ...
        if (this.grayscale) {
            ShaderHelper.setShader((SpriteBatch)sb, (ShaderHelper.Shader)ShaderHelper.Shader.GRAYSCALE);
        }
        ...
        if (this.hb.hovered) {
            ...
            if ((float)InputHelper.mX < 1400.0f * Settings.scale) {
                if (CardCrawlGame.mainMenuScreen.screen == MainMenuScreen.CurScreen.RELIC_VIEW && (float)InputHelper.mY < (float)Settings.HEIGHT / 5.0f) {
                    TipHelper.renderGenericTip((float)((float)InputHelper.mX + 60.0f * Settings.scale), (float)((float)InputHelper.mY + 100.0f * Settings.scale), (String)LABEL[3], (String)unlockReq);
                } else {
                    TipHelper.renderGenericTip((float)((float)InputHelper.mX + 60.0f * Settings.scale), (float)((float)InputHelper.mY - 50.0f * Settings.scale), (String)LABEL[3], (String)unlockReq);
                }
            } else {
                TipHelper.renderGenericTip((float)((float)InputHelper.mX - 350.0f * Settings.scale), (float)((float)InputHelper.mY - 50.0f * Settings.scale), (String)LABEL[3], (String)unlockReq);
            }
    ...
    public void render(SpriteBatch sb, boolean renderAmount, Color outlineColor) {
        ...
        if (AbstractDungeon.screen != null && AbstractDungeon.screen == AbstractDungeon.CurrentScreen.NEOW_UNLOCK) {
            if (this.largeImg == null) {
                sb.draw(this.img, this.currentX - 64.0f, this.currentY - 64.0f, 64.0f, 64.0f, 128.0f, 128.0f, Settings.scale * 2.0f + MathUtils.cosDeg((float)(System.currentTimeMillis() / 5L % 360L)) / 15.0f, Settings.scale * 2.0f + MathUtils.cosDeg((float)(System.currentTimeMillis() / 5L % 360L)) / 15.0f, this.rotation, 0, 0, 128, 128, false, false);
            } else {
                sb.draw(this.largeImg, this.currentX - 128.0f, this.currentY - 128.0f, 128.0f, 128.0f, 256.0f, 256.0f, Settings.scale * 1.0f + MathUtils.cosDeg((float)(System.currentTimeMillis() / 5L % 360L)) / 30.0f, Settings.scale * 1.0f + MathUtils.cosDeg((float)(System.currentTimeMillis() / 5L % 360L)) / 30.0f, this.rotation, 0, 0, 256, 256, false, false);
            }
        } else {
            sb.draw(this.img, this.currentX - 64.0f, this.currentY - 64.0f, 64.0f, 64.0f, 128.0f, 128.0f, this.scale, this.scale, this.rotation, 0, 0, 128, 128, false, false);
        ...
        if (this.hb.hovered && !CardCrawlGame.relicPopup.isOpen) {
            if (!this.isSeen) {
                if ((float)InputHelper.mX < 1400.0f * Settings.scale) {
                    TipHelper.renderGenericTip((float)((float)InputHelper.mX + 60.0f * Settings.scale), (float)((float)InputHelper.mY - 50.0f * Settings.scale), (String)LABEL[1], (String)MSG[1]);
                } else {
                    TipHelper.renderGenericTip((float)((float)InputHelper.mX - 350.0f * Settings.scale), (float)((float)InputHelper.mY - 50.0f * Settings.scale), (String)LABEL[1], (String)MSG[1]);
                }
                return;
            }
            this.renderTip(sb);
    ...
    public void renderCounter(SpriteBatch sb, boolean inTopPanel) {
        if (this.counter > -1) {
            if (inTopPanel) {
                FontHelper.renderFontRightTopAligned((SpriteBatch)sb, (BitmapFont)FontHelper.topPanelInfoFont, (String)Integer.toString(this.counter), (float)(offsetX + this.currentX + 30.0f * Settings.scale), (float)(this.currentY - 7.0f * Settings.scale), (Color)Color.WHITE);
            } else {
                FontHelper.renderFontRightTopAligned((SpriteBatch)sb, (BitmapFont)FontHelper.topPanelInfoFont, (String)Integer.toString(this.counter), (float)(this.currentX + 30.0f * Settings.scale), (float)(this.currentY - 7.0f * Settings.scale), (Color)Color.WHITE);
            }
        }
    }
    ...
    public void renderOutline(Color c, SpriteBatch sb, boolean inTopPanel) {
        sb.setColor(c);
        if (AbstractDungeon.screen != null && AbstractDungeon.screen == AbstractDungeon.CurrentScreen.NEOW_UNLOCK) {
            sb.draw(this.outlineImg, this.currentX - 64.0f, this.currentY - 64.0f, 64.0f, 64.0f, 128.0f, 128.0f, Settings.scale * 2.0f + MathUtils.cosDeg((float)(System.currentTimeMillis() / 5L % 360L)) / 15.0f, Settings.scale * 2.0f + MathUtils.cosDeg((float)(System.currentTimeMillis() / 5L % 360L)) / 15.0f, this.rotation, 0, 0, 128, 128, false, false);
    ...
    public void renderBossTip(SpriteBatch sb) {
        TipHelper.queuePowerTips((float)((float)Settings.WIDTH * 0.63f), (float)((float)Settings.HEIGHT * 0.63f), this.tips);
    }

    public void renderTip(SpriteBatch sb) {
        if ((float)InputHelper.mX < 1400.0f * Settings.scale) {
            if (CardCrawlGame.mainMenuScreen.screen == MainMenuScreen.CurScreen.RELIC_VIEW) {
                TipHelper.queuePowerTips((float)(180.0f * Settings.scale), (float)((float)Settings.HEIGHT * 0.7f), this.tips);
                ...
                TipHelper.queuePowerTips((float)((float)InputHelper.mX + 60.0f * Settings.scale), (float)((float)InputHelper.mY + 180.0f * Settings.scale), this.tips);
            } else if (AbstractDungeon.player != null && AbstractDungeon.player.hasRelic(this.relicId)) {
                TipHelper.queuePowerTips((float)((float)InputHelper.mX + 60.0f * Settings.scale), (float)((float)InputHelper.mY - 30.0f * Settings.scale), this.tips);
            } else if (AbstractDungeon.screen == AbstractDungeon.CurrentScreen.COMBAT_REWARD) {
                TipHelper.queuePowerTips((float)(360.0f * Settings.scale), (float)((float)InputHelper.mY + 50.0f * Settings.scale), this.tips);
            } else {
                TipHelper.queuePowerTips((float)((float)InputHelper.mX + 50.0f * Settings.scale), (float)((float)InputHelper.mY + 50.0f * Settings.scale), this.tips);
            }
        } else {
            TipHelper.queuePowerTips((float)((float)InputHelper.mX - 350.0f * Settings.scale), (float)((float)InputHelper.mY - 50.0f * Settings.scale), this.tips);
        }
    }
    ...
    protected void initializeTips() {
        ...
        while (desc.hasNext()) {
            ...
            this.tips.add(new PowerTip(TipHelper.capitalize((String)s), (String)GameDictionary.keywords.get(s)));
        }
        desc.close();
    }
    ...
    public static void updateOffsetX() {
        ...
        if (offsetX != target) {
            offsetX = MathHelper.uiLerpSnap((float)offsetX, (float)target);
        }
    }

    public void loadLargeImg() {
        if (this.largeImg == null) {
            this.largeImg = ImageMaster.loadImage((String)(L_IMG_DIR + this.imgUrl));
        }
    }
