...
//    private Color baseColor = ColorDatabase.DEFAULT_RED;
    ...
    protected void updateWidget() {
        ...
        UIStrings uiStrings = MapMarks.getLegendUIStrings();
        if (hb.hovered) {
            if (EaselInputHelper.isShiftPressed()) {
                TipHelper.renderGenericTip(1500.0f * Settings.xScale,
                        (getBottom() - 20.0f) * Settings.scale,
                        LocalizationHelper.getDictString(uiStrings, LocalizationConstants.Legend.TipHeader),
                        LocalizationHelper.getDictString(uiStrings, LocalizationConstants.Legend.TipBody));
            }
            else {
                TipHelper.renderGenericTip(1500.0f * Settings.xScale,
                        270.0f * Settings.scale,
                        LocalizationHelper.getDictString(uiStrings, LocalizationConstants.Legend.TipHeaderSuccinct),
                        LocalizationHelper.getDictString(uiStrings, LocalizationConstants.Legend.TipBodySuccinct));
            }
        }
